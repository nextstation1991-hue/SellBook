...
let ordersRealtimeReloadTimer = null;

const ORDERS_THEME_STORAGE_KEY = 'theme';
...
function toggleTheme() {
  ...
  localStorage.setItem(ORDERS_THEME_STORAGE_KEY, isDark ? 'dark' : 'light');
}
...
function subscribeRealtime() {
  ...
  realtimeChannel = supabaseClient
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, payload => {
      ...
      scheduleOrdersRealtimeRefresh({ orders: true });
    })
    .on('postgres_changes', { event: '*', schema: 'public', table: 'order_items' }, payload => {
      console.log('Order items Realtime change:', payload.eventType);
      scheduleOrdersRealtimeRefresh({ orders: true });
    })
    .on('postgres_changes', { event: '*', schema: 'public', table: 'marketplace_import_batches' }, payload => {
      console.log('Marketplace batch Realtime change:', payload.eventType);
      scheduleOrdersRealtimeRefresh({ marketplace: true });
    })
    .on('postgres_changes', { event: '*', schema: 'public', table: 'marketplace_import_rows' }, payload => {
      console.log('Marketplace row Realtime change:', payload.eventType);
      scheduleOrdersRealtimeRefresh({ marketplace: true });
    })
    .subscribe();
}

function scheduleOrdersRealtimeRefresh({ orders = false, marketplace = false } = {}, delay = 180) {
  clearTimeout(ordersRealtimeReloadTimer);
  ordersRealtimeReloadTimer = setTimeout(() => {
    if (orders) {
      loadOrders(true);
    }
    if (marketplace) {
      ['shopee', 'tiktok'].forEach((platform) => {
        loadMarketplaceImportData(platform, true);
      });
    }
  }, delay);
}

function setupOrdersAppSync() {
  window.addEventListener('storage', (event) => {
    if (event.key === ORDERS_THEME_STORAGE_KEY) {
      initTheme();
    }
  });
}
