...
let adminsSyncChannel = null;

const ADMINS_THEME_STORAGE_KEY = 'theme';
const ADMINS_SYNC_STORAGE_KEY = 'ratt_admin_accounts_sync';
const ADMINS_SYNC_CHANNEL_NAME = 'ratt-admin-accounts';
...
function toggleTheme() {
  ...
  localStorage.setItem(ADMINS_THEME_STORAGE_KEY, isDark ? 'dark' : 'light');
}
...
function emitAdminsSync(type, payload = {}) {
  const message = { type, payload, sentAt: Date.now() };

  try {
    if (!adminsSyncChannel && typeof BroadcastChannel !== 'undefined') {
      adminsSyncChannel = new BroadcastChannel(ADMINS_SYNC_CHANNEL_NAME);
    }
    adminsSyncChannel?.postMessage(message);
  } catch {}

  try {
    localStorage.setItem(ADMINS_SYNC_STORAGE_KEY, JSON.stringify(message));
  } catch {}
}

function handleAdminsSyncMessage(message) {
...
function setupAdminsAppSync() {
  if (typeof BroadcastChannel !== 'undefined') {
    try {
      adminsSyncChannel = new BroadcastChannel(ADMINS_SYNC_CHANNEL_NAME);
      adminsSyncChannel.onmessage = (event) => handleAdminsSyncMessage(event.data);
    } catch {}
  }

  window.addEventListener('storage', (event) => {
    if (event.key === ADMINS_THEME_STORAGE_KEY) {
      initTheme();
      return;
    }

    if (event.key === ADMINS_SYNC_STORAGE_KEY && event.newValue) {
      try {
        handleAdminsSyncMessage(JSON.parse(event.newValue));
      } catch {}
    }
  });
}
...
async function promoteAdmin(targetEmail) {
  ...
  try {
    ...
    emitAdminsSync('accounts-changed', { email, isAdmin: true });
...
async function confirmRemoveAdmin() {
  ...
  try {
    ...
    emitAdminsSync('accounts-changed', { email: removingAdminEmail, isAdmin: false });
    await loadAdminAccounts();
  } catch (err) {
